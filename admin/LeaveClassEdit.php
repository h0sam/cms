...
//$leave_classes_query=mysql_query("SELECT * FROM leave_classes WHERE lcid=$leave_class_id")or die("Leave Classes Query: ".mysql_errno($db) . ": " . mysql_error($db) . "\n");
//$leave_classes=mysql_fetch_array($leave_classes_query);

$leave_classes = $db->select("leave_classes", "lcid=$leave_class_id");
foreach ($leave_classes as $row){
	$leave_class_name = $row["name"];
...
<body>
		...
		<section id="content">
			...
			Looks good with tableless, schematic markup.</p>
				...
				<form id="class_edit" action="submit.php?page=lclasses&action=edit&lcid=<?php echo $leave_class_id; ?>" method="post" autocomplete="off">
                ...
                <fieldset>
						...
						<section><label for="leave_class_name">Leave Class Name</label>
							<div><input type="text" id="leave_class_name" name="leave_class_name" required data-errortext="Leave Class name is mandatory!" value="<?php print $leave_class_name; ?>">
                        ...
                        <section><label for="days_count">Days Allowed</label>
							<div><input id="days_count" name="days_count" type="number" class="integer" required="required" value="<?php print $days_count; ?>">
