...
<body>
		...
		<section id="content">
			...
			Looks good with tableless, schematic markup.</p>
				...
				<form id="create_leave" action="admin/submit.php?page=leaves&action=new" method="post" >
                ...
                <fieldset>
                        ...
                        <section>
							...
							$leave_classes_result = $db->select("leave_classes ORDER BY lcid ASC");
								foreach ($leave_classes_result as $row){
									...
									$class_name=$row["name"];
							?>
								<input type="radio" id="leave_class" name="leave_class" value="<?php echo $class_id; ?>" <?php if ($class_name == "أجازة إعتيادية" ): ?>checked="checked" <?php endif; ?>><label for="<?php echo $class_name; ?>"><?php echo $class_name; ?></label>
						...
						<section><label for="start_date">Start Date</label>
							<div><input id="start_date" name="start_date" type="text" class="date" required="required">
                       ...
                       <section><label for="duration">Duration</label>
							<div><input id="duration" name="duration" type="number" class="integer" required="required">
